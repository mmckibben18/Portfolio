...
f_house <- filter(housing, between(Sale.Price, 400000, 500000), .by = Sale.Price) %>%
...
# Splitting the data 
select(housing, c(Sale.Price, square_feet_total_living, sq_ft_lot)) 

# Organizing the data
arrange(housing, desc(square_feet_total_living), .by_group = TRUE, .locale = "en")
arrange(housing, desc(sq_ft_lot), .by_group = TRUE, .locale = "en")
...
g_sale_price <- group_by(housing, Sale.Price, square_feet_total_living, sq_ft_lot, .add = TRUE, .drop = TRUE)
g_sale_price
...
# rbind
highest_priced <- arrange(housing, desc(Sale.Price), .by_group = TRUE, .locale = "en")
lowest_priced <- arrange(housing, desc(Sale.Price), .by_group = TRUE, .locale = "en")
top_and_bottom <- rbind(highest_priced %>% slice(1:5), lowest_priced %>% slice((nrow(lowest_priced)-4):nrow(lowest_priced)))
top_and_bottom

# cbind
sale_price <- housing$Sale.Price
sq_footage_interior <- housing$square_feet_total_living
cbind(sale_price, sq_footage_interior)
...
string_5.2 <- paste(string_5.2.1, string_5.2.2, string_5.2.3, string_5.2.4, string_5.2.5, collapse = " ")
string_5.2